...
export default function EditStudentForm({ schoolId, student, onSuccess, onCancel }: EditStudentFormProps) {
  const { showSuccess, showError } = useToast();
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState({
    firstName: student.firstName,
    lastName: student.lastName,
    birthdate: student.birthdate.split('T')[0], // Format YYYY-MM-DD pour input date
    sex: student.sex,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsLoading(true);
      await adminService.updateSchoolStudent(schoolId, student.id, formData);
      showSuccess('Élève modifié avec succès');
      onSuccess();
    } catch (error: any) {
      showError(error.response?.data?.message || 'Erreur lors de la modification de l\'élève');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-3xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500">ID: {student.uniqueId}</p>
          </div>
          <button
            onClick={onCancel}
